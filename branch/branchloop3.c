...
int main(int argc, char* argv[]) {
	...
	int index = atoi(argv[3]);		// number inputed
	...
	{
		char temp1 = firstName[i];
		firstName[i] = firstName[count1 - 1 - i];
		firstName[count1 - 1 - i] = temp1; 
	}

	for(i = 0; i < count2 / 2; i++)
	{
		char temp2 = lastName[i];
		lastName[i] = lastName[count2 - 1 - i];
		lastName[count2 - 1 - i] = temp2;
	}

	//printf("There are %d letters in your name.\n", count1 + count2);

	printf("%d %s %s\n", count1 + count2, firstName, lastName);
